...
fs模块是node-karin的文件系统工具集合，提供了一系列便捷的文件操作函数，包括文件读写、路径处理、数据转换等功能。

## 命名空间使用

fs模块可以作为命名空间整体导入，也可以直接导入其中的函数：
...
json模块提供了读写JSON文件的便捷函数，支持同步和异步操作。
...
yaml模块提供了读写YAML文件的工具函数，同时支持注释处理和高级编辑功能。
...
changelog模块提供了用于处理更新日志(CHANGELOG.md)的工具函数。
