...
// 通过索引获取
const botInstance1 = bot.getBot(0)

// 通过协议获取
const botInstance2 = bot.getBot('onebot', true)

// 通过机器人ID获取
const botInstance3 = bot.getBot('123456789')
```
 ...
 * @returns 注册的Bot列表
 */
const botsList = bot.getAllBotList()
...
const botIndex = bot.registerBot(communication, botInstance)
```
...
// 通过索引卸载
const success1 = bot.unregisterBot('index', 1)

// 通过机器人ID卸载
const success2 = bot.unregisterBot('selfId', '123456789')

// 通过机器人ID和地址卸载
const success3 = bot.unregisterBot('address', '123456789', 'ws://example.com')
```
...
```typescript
import { bot, segment } from 'node-karin'
...
// 发送文本消息
const result1 = await bot.sendMsg('123456789', {
  scene: 'group',
  peer: '群号',
  userId: '发送者ID'
}, '这是一条消息')
...
// 发送复杂消息元素
const result2 = await bot.sendMsg('123456789', {
  scene: 'private',
  peer: '私聊对象ID'
}, [
  segment.text('Hello'),
...
// 使用选项
const result3 = await bot.sendMsg('123456789', {
  scene: 'group',
  peer: '群号'
}, '这条消息将在5秒后自动撤回', {
  recallMsg: 5, // 5秒后撤回
...
// 获取所有机器人实例
const allBots = bot.getAllBot()
console.log(`当前共有 ${bot.getBotCount()} 个机器人实例`)

// 通过ID获取特定机器人
const myBot = bot.getBot('123456789')
if (myBot) {
  console.log(`找到机器人: ${myBot.account.name}`)
}

// 向特定群组发送消息
await bot.sendMsg('123456789', {
  scene: 'group',
  peer: '10000000'
}, [
...
// 发送一条5秒后自动撤回的消息
const result = await bot.sendMsg('123456789', {
  scene: 'private',
  peer: '10000000'
}, [
...
const checkBotsStatus = () => {
  ...
  botIds.forEach(id => {
    const botInstance = bot.getBot(id)
    if (botInstance) {
